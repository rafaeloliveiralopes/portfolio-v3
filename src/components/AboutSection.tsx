      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-6">
            ...
            <span className="text-gradient-primary">{t('about.title').split(' ')[1]} {t('about.title').split(' ')[2]}</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="space-y-6">
            <div className="prose prose-invert max-w-none">
              <p className="text-lg text-muted-foreground leading-relaxed">
                ...
                <span className="text-primary font-semibold">{t('about.story1').split('**')[1]}</span>
                {t('about.story1').split('**')[2]}
                <span className="text-secondary">{t('about.story1').split('**')[3]}</span>
                {t('about.story1').split('**')[4]}
                <span className="text-primary">{t('about.story1').split('**')[5]}</span>
                {t('about.story1').split('**')[6]}
              </p>
              
              <p className="text-lg text-muted-foreground leading-relaxed">
                {t('about.story2').split('**')[0]}
                <span className="text-gradient-primary font-semibold">{t('about.story2').split('**')[1]}</span>
            ...
            <div className="grid md:grid-cols-2 gap-4 mt-12">
              {[
                { 
                  ...
                  items: t('about.skills.development.items', { returnObjects: true }) as string[]
                },
                { 
                  icon: Palette, 
                  title: t('about.skills.design.title'), 
                  items: t('about.skills.design.items', { returnObjects: true }) as string[]
                },
                { 
                  icon: Lightbulb, 
                  title: t('about.skills.soft.title'), 
                  items: t('about.skills.soft.items', { returnObjects: true }) as string[]
                },
                { 
                  icon: Target, 
                  title: t('about.skills.focus.title'), 
                  items: t('about.skills.focus.items', { returnObjects: true }) as string[]
                }
              ].map((skill, index) => {
          ...
          <div className="relative">
            <h3 className="text-2xl font-bold mb-8 text-center lg:text-left">
              {t('about.timeline.title').split(' ')[0]} <span className="text-gradient-secondary">{t('about.timeline.title').split(' ')[1]}</span>
            </h3>
